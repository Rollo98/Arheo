...
class ViewAccount extends Component {
  ...
  async componentWillMount() {
    if ((this.props.userName === null) || (this.props.userName === undefined)) {
      ...
      if (Response) {
        ...
        if (roles.length > 0)
          roles.map(n => { checkboxesOptions[n] = true })
      ...
      const jwtToken = localStorage.getItem("zeBilet");
      Axios.defaults.headers.common["Authorization"] = jwtToken;
      let Response = await Axios.get(`http://localhost:5000/${this.props.userName}`)
      if (Response) {
        const roles = Object.values(Response.data.foundUser[0].role);
        let checkboxesOptions = this.state.checkboxesOptions
        if (roles.length > 0)
            ...
            {!edit ? <>
              {this.state.checkboxesOptions[n] ?
                <div key={key} style={{ marginLeft: "10px", marginTop: "10px" }}>
                  <Checkbox label={n} isSelected={this.state.checkboxesOptions[n]} key={n} />
                </div> : null
              }</> : <>
                <div key={key} style={{ marginLeft: "10px", marginTop: "10px" }}>
                  <Checkbox label={n} isSelected={this.state.checkboxesOptions[n]} onCheckboxChange={this.handleCheckboxChange} key={n} />
                </div>
              </>}
          </>
        )
 ...
 async sendDeleteAcc() {
  ...
  render() {
    return (
      !this.state.show ? (<>
        {this.props.userName ? <Redirect to="/dashboard" /> : <Redirect to="/" />}
      </>) : (
          <>
            <Modal show={this.state.show} onHide={this.handleClose} >
              ...
              <Modal.Body>{this.state.edit ? <>
                <div className='input-group' >
                  ...
                  <input className='form-control' value={this.state.lastName} onChange={(e) => { this.setState({ lastName: e.target.value }) }} />
                </div>
                <div className='input-group' >
                  <span className='input-group-text' id='inputGroup-sizing-sm'>Prenume:</span>
                  <input className='form-control' value={this.state.firstName} onChange={(e) => { this.setState({ firstName: e.target.value }) }} />
                </div>
                <div className='input-group' >
                ...
                <div className='input-group' >
                  ...
                  {this.props.userName ? this.roleRender(true) : this.roleRender(false)}
                </div>
                <div className='input-group' >
                  <span className='input-group-text' id='inputGroup-sizing-sm'>Parola</span>
                  <input className='form-control' value={this.state.password} onChange={(e) => { this.setState({ password: e.target.value }) }} />
                </div>
              </> : <>
                  <div className='input-group' >
              ...
              <Modal.Footer>
                ...
                {!this.state.edit ? <>
                  <button variant='primary' type='button' className='btn' onClick={() => { this.toggleEdit() }}>
                    ...
                    <button variant='primary' type='button' className='btn btn-danger ' onClick={() => { this.toggleEdit(); }}>
                      Renunta
                </button>
                  </>}
...
export default
  connect(MapStateToProps)(ViewAccount)
